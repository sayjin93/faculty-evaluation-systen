...
function generateRandomPassword() {
  ...
  for (let i = 0; i < additionalLength; i += 1) {
    mandatoryCharacters.push(allCharacters[Math.floor(Math.random() * allCharacters.length)]);
  }

  for (let i = mandatoryCharacters.length - 1; i > 0; i -= 1) {
    const j = Math.floor(Math.random() * (i + 1));
    [mandatoryCharacters[i], mandatoryCharacters[j]] = [mandatoryCharacters[j], mandatoryCharacters[i]];
  }
